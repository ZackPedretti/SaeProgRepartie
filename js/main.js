(() => {
  ...
  var urlApi = "https://transport.data.gouv.fr/gbfs/nancy/gbfs.json";
  function getInfoLink(n) {
    return __async(this, null, function* () {
      try {
        ...
        console.error(`Erreur lors de la r\xE9cup\xE9ration du lien d'info : ${error}`);
        return null;
      }
    });
  }
  function getSystemInfoLink() {
    return __async(this, null, function* () {
      return yield getInfoLink(0);
    });
  }
  function getStationInfoLink() {
    return __async(this, null, function* () {
      return yield getInfoLink(1);
    });
  }
  function getStationStatusLink() {
    return __async(this, null, function* () {
      return yield getInfoLink(2);
    });
  }
  function getAllVelibs() {
    return __async(this, null, function* () {
      const velibs = [];
      const systemInfoLink = yield getSystemInfoLink();
      const stationInfoLink = yield getStationInfoLink();
      const stationStatusLink = yield getStationStatusLink();
      if (!systemInfoLink || !stationInfoLink || !stationStatusLink) {
        console.error("Impossible de r\xE9cup\xE9rer tous les liens n\xE9cessaires.");
        return velibs;
      }
      try {
        const systemResponse = yield fetch(systemInfoLink);
        if (!systemResponse.ok) {
          throw new Error(`Erreur HTTP ! statut : ${systemResponse.status}`);
        }
        const systemData = yield systemResponse.json();
        const { name: systemName, language, system_id: systemId } = systemData.data;
        const stationInfoResponse = yield fetch(stationInfoLink);
        ...
        const stationStatusResponse = yield fetch(stationStatusLink);
        ...
        console.error(`Erreur lors de la r\xE9cup\xE9ration des stations de v\xE9libs : ${e}`);
      }
      return velibs;
    });
  }
    ...
    (JSON2) => {
      ...
      for (var key in JSON2) {
        if (/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/.test(key)) {
          document.getElementById("cartesMeteo").innerHTML += `<div id="carte">
                <h4>Date : ${key}</h4>
                <img src="images/meteo/neige.svg" alt="logo meteo" class="carteImage">
                <p>Temp\xE9rature : ${Math.round(JSON2[key].temperature.sol - 273.15)} \xB0C</p>
            </div>`;
        }
      }
    }
  );
  var emptyStationVelibIcon = new mapIcons({ iconUrl: "images/emptyStationVelibIcon.svg" });
  ...
  getAllVelibs().then((stationsVelib) => {
    stationsVelib.forEach(function(station, index, array) {
      ...
      L.marker([station.lat, station.lon], { icon: iconStation }).addTo(stationMarkers).bindPopup(
        `<h1> ${station.stationName} </h1>
            ...
            <p> Nombre de v\xE9los disponibles : ${station.bikesAvailable} </p>
            <p> Nombre de docks disponibles : ${station.docksAvailable} </p>
            <p> Capacit\xE9 : ${station.capacity} </p>`
      );
    });
  });
  ...
  var mapType2 = L.tileLayer("https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png", {
    maxZoom: 20,
    attribution: "\xA9 OpenStreetMap contributors, Tiles style by Humanitarian OpenStreetMap Team hosted by OpenStreetMap France"
  });
